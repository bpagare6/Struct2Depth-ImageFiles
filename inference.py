...
def _run_inference(output_dir="/home/bhushan/Priority-Based-Alert/nvidia/output/",
                   file_extension='png',
                   depth=True,
                   egomotion=False,
                   model_ckpt=None,
                   input_dir="/home/bhushan/Priority-Based-Alert/nvidia/input/",
                   ...
                   inference_mode=INFERENCE_MODE_TRIPLETS,
        ...
        if depth:
            ...
            for i in range(len(im_files)):
                ...
                if inference_mode == INFERENCE_MODE_SINGLE:
                    ...
                    im = util.load_image(
                        im_files[i], resize=(img_width * 3, img_height))
                ...
                if len(im_batch) == batch_size or i == len(im_files) - 1:
                    ...
                    # Fill up batch.
                    for _ in range(batch_size - len(im_batch)):
                    ...
                    for j in range(len(im_batch)):
                        ...
                        util.save_image(
                            output_vis, visualization, file_extension)
                    im_batch = []
        ...
        if egomotion:
            if inference_mode == INFERENCE_MODE_SINGLE:
                ...
                for i in range(len(im_files)):
                    ...
                    print("*********************    Here     ******************")
                    if use_masks:
                        segmentation.get_output(im_files[i])
                    ...
                    if len(input_image_seq) < seq_length:
                        print("*********************    Here     ******************")
                        continue
                    # Remove oldest entry.
                    if len(input_image_seq) > seq_length:
                    ...
                    input_image_stack = np.expand_dims(
                        input_image_stack, axis=0)
                    if use_masks:
                    ...
                    for j in range(seq_length - 1):
                        egomotion_str.append(
                            ','.join([str(d) for d in est_egomotion[j]]))
                    current_output_handle.write(
                        str(i) + ' ' + ' '.join(egomotion_str) + '\n')
                    print("written *********", egomotion_str, "************")
                if current_output_handle is not None:
                    current_output_handle.close()
            elif inference_mode == INFERENCE_MODE_TRIPLETS:
                written_before = []
                for i in range(len(im_files)):
                    ...
                    egomotion_1_2 = ','.join([str(d)
                                              for d in est_egomotion[0]])
                    egomotion_2_3 = ','.join([str(d)
                                              for d in est_egomotion[1]])

                    output_filepath = output_dir + "/egomotion.txt"
